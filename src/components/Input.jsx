      ...
      <div className="relative">
        {isSelect ? (
          <select
            ...
            className="w-full bg-white outline-none border border-gray-300 rounded-md py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:border-blue-500 appearance-none"
          >
        ...
        {type === 'password' && (
          <span className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer">
            {showPassword ? (
              <EyeOff size={20} className="text-purple-600" onClick={toggle} />
            ) : (
              <Eye size={20} className="text-slate-400" onClick={toggle} />
            )}
          </span>
        )}
      </div>
